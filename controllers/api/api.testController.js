...
module.exports = {
   ...
   takeTest: async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(userId, { $inc: { score: score } }, { new: true });

        console.log("User details after all answers correct:", user);

        res.status(200).json({ message: 'Test completed', user, session });
    } catch (error) {
